...
export default function UserDashboard() {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (adminAuth.id === "admin" && adminAuth.pass === "pune77") {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] text-slate-900 selection:bg-indigo-100 overflow-x-hidden">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute bottom-[0%] right-[-5%] w-[40%] h-[40%] bg-purple-100/40 blur-[120px] rounded-full" />
      </div>

      {/* Navbar */}
      <nav className="sticky top-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-8 py-5">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          
          {/* Left Side: Home & Branding */}
          <div className="flex items-center gap-6">
            <Link href="/" className="flex items-center gap-2 text-slate-400 hover:text-indigo-600 transition group">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-black tracking-tighter text-slate-900 italic uppercase">EduStream</span>
            </div>
          </div>

          {/* Right Side: Admin & Enroll */}
          <div className="flex items-center gap-8">
            ...
            <button className="bg-slate-900 text-white text-[10px] font-black px-8 py-3.5 rounded-2xl hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 uppercase tracking-widest">
              Enroll Now
            </button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {showLogin && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <button onClick={() => setShowLogin(false)} className="absolute top-8 right-8 text-slate-300 hover:text-slate-900 transition">
                <X size={24} />
              </button>
              ...
              <div className="text-center mb-10">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Credentials Required</p>
              </div>

              <form onSubmit={handleAdminLogin} className="space-y-4">
                <input 
                  type="text" placeholder="ADMIN ID" 
                  className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-indigo-600 font-bold uppercase text-xs"
                ...
                <input 
                  ...
                  required
                />
                <button type="submit" className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-950 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center gap-2">
      ...
      {/* Hero Section */}
      <header className="px-6 pt-28 pb-24 max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }}>
          <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-full text-xs font-black text-indigo-600 mb-8 uppercase tracking-widest">
            <Sparkles size={16} /> Premium Learning Experience
          </div>
          <h1 className="text-7xl md:text-8xl font-black tracking-tight text-slate-900 mb-8 leading-[0.9] uppercase italic">
          ...
          <div className="flex gap-6">
            <button className="bg-indigo-600 text-white px-10 py-5 rounded-[2rem] font-black text-lg hover:scale-105 transition shadow-2xl shadow-indigo-300 flex items-center gap-3">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="relative">
           <div className="w-full aspect-square rounded-[4rem] bg-gradient-to-br from-slate-900 to-slate-800 shadow-2xl flex items-center justify-center relative overflow-hidden">
      ...
      <main className="px-8 pb-32 max-w-7xl mx-auto">
        <h2 className="text-4xl font-black text-slate-900 mb-16 uppercase italic underline decoration-indigo-500 decoration-8 underline-offset-8">Catalog</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {courses.map((course: any, idx) => (
            <motion.div
              ...
              className="group"
            >
              <div className="bg-white rounded-[3rem] p-4 border border-slate-200/60 shadow-xl hover:shadow-2xl transition-all duration-700">
                <div className={`h-64 rounded-[2rem] overflow-hidden relative mb-8 flex items-center justify-center transition-all duration-500 ${
                ...
                <div className="px-4 pb-4">
                  ...
                  <h3 className="text-3xl font-black text-slate-900 mb-6 group-hover:text-indigo-600 transition-colors leading-[0.9] uppercase italic tracking-tighter">
                    {course.title}
                  </h3>
                  <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-50">
                    <div>
                      <span className="text-2xl font-black text-slate-900 block tracking-tighter">₹{course.price}</span>
